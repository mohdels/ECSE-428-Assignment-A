Feature: Update Textbook Published Year

  As a seller
  I want to be able to update the published year of a used textbook post
  ...
  Scenario: Update a published year number (successful scenario)
    ...
    And the user updates the published year of the textbook
  ...
  Scenario: Try to update textbook published year with an empty field
    Given the user is logged in to their account
    And editing a post they created
    When the user tries to update the textbook's published year with an empty field
