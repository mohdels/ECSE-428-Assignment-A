Feature: Update Textbook Edition Number

  As a seller
  I want to be able to update the edition number of a used textbook post
  ...
  Scenario: Update a textbook edition number (successful scenario)
    ...
    And the user updates the edition number of the textbook
  ...
  Scenario: Try to update edition number with an empty field
    ...
    When the user tries to update the textbook's edition number with an empty field
