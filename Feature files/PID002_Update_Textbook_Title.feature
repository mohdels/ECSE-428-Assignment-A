Feature: Update Textbook Title

  As a seller
  I want to be able to update the title of a used textbook post
  so that it accurately reflects the book's information.

  Scenario: Update a textbook title (successful scenario)
    ...
    And the user updates the title of the textbook
  ...
  Scenario: Try to update textbook title with an empty field
    Given the user is logged in to their account
    And editing a post they created
    When the user tries to update the textbook's title with an empty field
