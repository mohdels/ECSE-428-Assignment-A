Feature: Update Trade Textbook Published Year

  As a seller
  I want to be able to update the published year of a textbook I want to trade mine for
  so that it accurately reflects the book's information.

  Scenario: Update a published year number (successful scenario)
    ...
    And the user updates the published year of the trade textbook
    And the user clicks the "Save Changes" button
    Then the user should be redirected back to the textbook listing page
    And the updated published year should be displayed for the textbook post
    And the user should receive a confirmation that the changes have been saved successfully.

  Scenario: Try to update trade textbook published year with an empty field
    Given the user is logged in to their account
    And editing a post they created
    When the user tries to update the trade textbook's published year with an empty field
  ...
  Scenario: Try to update trade textbook published year with an invalid format
    Given the user is logged in to their account
    And editing a post they created
    When the user inputs a published year number which is not a positive integer
